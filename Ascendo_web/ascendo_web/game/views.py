...
from ascendo_web.rules.models import Rules
from ascendo_web.user_profile.models import Profile
...
class CodeView(View):
    def get(self, request):
        if request.user.is_authenticated:
            ...
            if not profile.code:
                context = {'code': q, 'form': ResponseForm(), 'comment': Rules.objects.all().last()}
